...
import { decodeProfile } from "@/lib/landlord-profile";
import { LandlordProfileView } from "@/components/landlord/LandlordProfileView";
import { TenantOnlyHeaderLink } from "@/components/listing/RoleAwareApplyCTA";

type PageProps = { params: { id: string } };

export function generateMetadata({ params }: PageProps) {
  const profile = decodeProfile(params.id);
  if (!profile) return { title: "Profile not found — Rentiff" };
  return {
    title: `${profile.name} — Rentiff`,
    description: profile.bio ?? `Verified landlord profile on Rentiff.`,
  };
}
...
export default function LandlordProfilePage({ params }: PageProps) {
      ...
      <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center px-4">
        <div className="max-w-md w-full rounded-2xl border border-white/10 bg-slate-900/60 backdrop-blur p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            The link may be malformed. Try recreating the profile.
          </p>
          <Link
  ...
  return (
    <main className="relative min-h-screen bg-slate-950 text-slate-100">
      <div
        aria-hidden
        className="pointer-events-none fixed inset-x-0 top-0 -z-0 h-[400px] bg-[radial-gradient(40%_50%_at_50%_-10%,rgb(16_185_129_/_0.12)_0%,transparent_70%)]"
      />
      ...
      <LandlordProfileView profile={profile} profileEncoded={params.id} />
    </main>
  );
}
