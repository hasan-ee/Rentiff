...
import { ListingApplicants } from "@/components/landlord/ListingApplicants";

type PageProps = { params: { id: string } };

export function generateMetadata({ params }: PageProps) {
  const listing = decodeListing(params.id);
  if (!listing) return { title: "Applications — Rentiff" };
  return { title: `${listing.title} — applications` };
}
...
export default function ApplicantsPage({ params }: PageProps) {
  ...
  return (
    <main className="relative min-h-screen bg-slate-950 text-slate-100">
      ...
      <ListingApplicants listing={listing} listingEncoded={params.id} />
    </main>
  );
}
